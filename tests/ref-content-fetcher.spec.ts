...
test('should copy reference content for chapter', async () => {
...
test('should copy reference content for verse', async () => {
...
test('should copy reference content for verse range', async () => {
...
test('should be case-insensitive for reference IDs', async () => {
  const reference = await fetchReferenceContent('59/PsA.23');
  expect(reference.name).toEqual('Psalms 23');
});

test('should fetch reference content by query', async () => {
...
test('reference name should reflect chosen language', async () => {
...
test('should handle spaces appropriately', async () => {
  const reference = await fetchReferenceContent('111/psa.23');
...
test('should add line breaks where appropriate', async () => {
  const reference = await fetchReferenceContent('111/psa.23');
...
test('should strip line breaks where appropriate', async () => {
  const reference = await fetchReferenceContent('111/psa.23', {
...
test('should display verse numbers correctly when stripping line breaks', async () => {
  const reference = await fetchReferenceContent('111/psa.23', {
...
test('should display verse numbers correctly with line breaks', async () => {
  const reference = await fetchReferenceContent('111/psa.23', {
    includeVerseNumbers: true,
...
test('should handle verse range labels (used by versions like the MSG)', async () => {
...
test('should handle range labels when verse at start of range is given', async () => {
...
test('should handle range labels when verse at end of range is given', async () => {
...
test('should handle range labels when verse in middle of range is given', async () => {
...
test('should fall back to node-fetch if native fetch() is unavailable', async () => {
